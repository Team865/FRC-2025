...
public class DriveToPose extends Command {
    ...
    static {
        ...
        driveTolerance.initDefault(Units.inchesToMeters(0.5));
    ...
    public DriveToPose(DriveSubsystem drive, Supplier<Pose2d> target) {
        this.drive = drive;
        this.target = target;
        robot = drive::getPose;
    ...
    public DriveToPose(DriveSubsystem drive, Supplier<Pose2d> target, Supplier<Pose2d> robot) {
        this(drive, target);
        this.robot = robot;
    }
    ...
    public void initialize() {
        ...
        ChassisSpeeds fieldVelocity = drive.getChassisSpeedsField();
    ...
    public void execute() {
        ...
        double ffScaler = MathUtil.clamp(
                (currentDistance - ffMinRadius.get()) / (ffMaxRadius.get() - ffMinRadius.get()), 0.0, 1.0);
        driveErrorAbs = currentDistance;
        driveController.reset(
                lastSetpointTranslation.getDistance(targetPose.getTranslation()),
                driveController.getSetpoint().velocity);
        double driveVelocityScalar =
                driveController.calculate(driveErrorAbs, 0.0) + driveController.getSetpoint().velocity * ffScaler;
        ...
        driveVelocity = driveVelocity.interpolate(linearFF.get().times(drive.getMaxLinearSpeedMetersPerSec()), linearS);
